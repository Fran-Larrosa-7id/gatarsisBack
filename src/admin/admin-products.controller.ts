...
@Controller('admin') export class AdminProductsController {
  ...
  @Delete('products/:productId') deleteProduct(@Param('productId', new ParseUUIDPipe()) id: string, @Req() r: AdminRequest) { return this.products.removeProduct(id, r.admin!.id); }
  ...
  @Delete('variants/:variantId') deleteVariant(@Param('variantId', new ParseUUIDPipe()) id: string, @Req() r: AdminRequest) { return this.products.removeVariant(id, r.admin!.id); }
